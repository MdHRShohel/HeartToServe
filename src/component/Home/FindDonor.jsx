  ...
  const {
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://heart-to-serve-server.vercel.app/donors?group=${group}`
      );
  ...
  return (
    <div className="container mx-auto">
      <div
        className="py-16"
        style={{
          background: `linear-gradient(#00000099, #24232338), url(https://i.ibb.co/8xN3qcZ/blood-donor.jpg)`,
      ...
      >
        <h2 className="text-center text-white font-bold text-3xl lg:text-5xl md:text-3xl drop-shadow-lg mb-10">
          Find Donor's
        </h2>
            ...
            >
              {donors.map((donor) => (
                <SwiperSlide className="px-5 py-6 bg-[#00000030] text-[#f8f8ff] border rounded shadow-sm  text-center font-semibold mb-4 cursor-pointer">
                  ...
                  <p className="text-sm text-neutral mb-2">{donor.email}</p>
